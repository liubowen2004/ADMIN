<template>
  <div class="fa">
    <div class="one">
      ...
      <el-button style="margin-left: 10px; color: white" color="#13ce66" :icon="Search">搜索</el-button>
      <el-button style="color: white" color="#ffba00" :icon="RefreshLeft">重置</el-button>
    </div>
    <div class="two">
      <div class="left">
        ...
        <exportVue></exportVue>
        <el-button type="info" :icon="Tickets">日志</el-button>
      </div>
      <div class="right">
    ...
    <el-table :data="timingTableData" style="width: 100%; margin-top: 20px">
      ...
      <el-table-column label="状态">
        <template #default="scope">
          <el-tag class="ml-2" type="warning">{{ scope.row.isPause == true ? "已暂停" : "已激活" }}</el-tag>
        </template>
      </el-table-column>
...
import { Calendar, Search, RefreshLeft, Plus, EditPen, Delete, Download, Refresh, Grid,Tickets } from "@element-plus/icons-vue"
...
const getTimingData = async () => {
  let res = await API.getTimingListApi(info)
  console.log(res, "调度")
  timingTableData.value = res.data.content
